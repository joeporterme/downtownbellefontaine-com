...
<div class="max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold text-theme-primary mb-6">Edit Event</h1>

    @if($event->status === 'rejected')
        <div class="bg-danger-100 dark:bg-danger-900/50 border border-danger-300 dark:border-danger-700 text-danger-700 dark:text-danger-300 px-4 py-3 rounded mb-6">
            <strong>This event was rejected.</strong> Please make the necessary changes and resubmit for approval.
        </div>
    @elseif($event->status === 'approved')
        <div class="bg-warning-100 dark:bg-warning-900/50 border border-warning-300 dark:border-warning-700 text-warning-700 dark:text-warning-300 px-4 py-3 rounded mb-6">
            <strong>Note:</strong> Making changes to an approved event will require re-approval.
        </div>
    @endif

    <form method="POST" action="{{ route('events.update', $event) }}" enctype="multipart/form-data" class="bg-theme-primary shadow border border-theme rounded-lg p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5"
                    class="mt-1 block w-full rounded-md border-theme bg-theme-primary text-theme-primary shadow-sm focus:border-primary-500 focus:ring-primary-500 px-3 py-2 border">{{ old('description', $event->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-danger-600 dark:text-danger-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if($event->featured_image)
                    <div class="mb-3">
                        <img src="{{ Storage::url($event->featured_image) }}" alt="Current featured image" class="w-full max-w-md h-auto rounded-md">
                        <p class="text-xs text-theme-tertiary mt-1">Current image - upload a new one to replace</p>
                    </div>
                @endif
            ...
            <div>
                ...
                <select name="business_id" id="business_id"
                    ...
                    @foreach($businesses as $business)
                        <option value="{{ $business->id }}" {{ old('business_id', $event->business_id) == $business->id ? 'selected' : '' }}>
                            {{ $business->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="city" id="city" value="{{ old('city', $event->city ?? 'Bellefontaine') }}"
                        class="mt-1 block w-full rounded-md border-theme bg-theme-primary text-theme-primary shadow-sm focus:border-primary-500 focus:ring-primary-500 px-3 py-2 border">
                </div>
                <div>
                    <label for="state" class="block text-sm font-medium text-theme-secondary">State</label>
                    <input type="text" name="state" id="state" value="{{ old('state', $event->state ?? 'OH') }}" maxlength="2"
                        class="mt-1 block w-full rounded-md border-theme bg-theme-primary text-theme-primary shadow-sm focus:border-primary-500 focus:ring-primary-500 px-3 py-2 border">
                </div>
                <div>
                    <label for="zip" class="block text-sm font-medium text-theme-secondary">ZIP</label>
                    <input type="text" name="zip" id="zip" value="{{ old('zip', $event->zip) }}"
                        class="mt-1 block w-full rounded-md border-theme bg-theme-primary text-theme-primary shadow-sm focus:border-primary-500 focus:ring-primary-500 px-3 py-2 border">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="url" name="more_info_url" id="more_info_url" value="{{ old('more_info_url', $event->more_info_url) }}" placeholder="https://..."
                    class="mt-1 block w-full rounded-md border-theme bg-theme-primary text-theme-primary shadow-sm focus:border-primary-500 focus:ring-primary-500 px-3 py-2 border">
                @error('more_info_url')
                    <p class="mt-1 text-sm text-danger-600 dark:text-danger-400">{{ $message }}</p>
                @enderror
    ...
    <div class="mt-6">
        <form method="POST" action="{{ route('events.destroy', $event) }}" onsubmit="return confirm('Are you sure you want to delete this event?');">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-danger-600 dark:text-danger-400 hover:text-danger-800 dark:hover:text-danger-300 text-sm">
                Delete Event
            </button>
        </form>
    </div>
